...
def replace_iterrows_itertuples(source: str) -> str:
    ...
    target_template = ast.Tuple(elts=[
        ast.Name(id="_"),
        ast.Name(id=parsing.Wildcard("new_target_id", str))
        ])
    iter_template = ast.Call(
        func=ast.Attribute(
            value=parsing.Wildcard("underlying_object", object),
    ...
    for node, new_target_id, underlying_object in parsing.walk_wildcard(root, template):
        ...
        unsupported_new_target_access_template = (
            ...
            ast.Subscript(value=ast.Attribute(value=new_target, attr=("loc", "iloc")), slice=(ast.Index(value=ast.Name), ast.Name)),
        )
        if any(any(parsing.walk(child, unsupported_new_target_access_template)) for child in node.body):
            continue
        ...
        n_new_target_mentions = len([
            child
            for child in node.body
            for _ in parsing.walk(child, new_target)
        ])
        if n_new_target_mentions != len(node_replacements):
            continue

        new_iter = ast.Call(
            func=ast.Attribute(value=underlying_object, attr="itertuples"),
            args=[], keywords=[])
